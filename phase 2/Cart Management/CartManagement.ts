...
function addb(name:string,price:string):void{
    cartsize();
     let locobject = JSON.parse(sessionStorage.getItem("locobject") || "[]");
...
function displaydata() {
    ...
    var endTable = "</table>"; 
    locobject.forEach(title => {tableContent += "<tr><td>"+title.name+"</td><td>"+title.price});
